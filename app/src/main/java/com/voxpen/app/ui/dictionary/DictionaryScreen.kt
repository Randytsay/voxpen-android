...
fun DictionaryScreenContent(
    onNavigateBack: () -> Unit,
    onNavigateToCorrectionMemory: () -> Unit = {},
    ...
    LaunchedEffect(showDuplicate) {
        if (showDuplicate) {
            Toast.makeText(
                context,
                context.getString(R.string.dictionary_duplicate),
                Toast.LENGTH_SHORT,
            ).show()
            ...
            text = {
                Text(
                    "加入常用人名、品牌、專有名詞，可提高語音辨識與 AI 潤稿的準確度。\n\n" +
                        "點擊 ☆ 可設為 ⭐ 重要詞。重要詞會優先提供給語音辨識與 AI 潤稿。\n\n" +
                        "個人學習記憶則會另外記住你曾經手動修正過的『錯詞 → 正詞』。",
                )
            },
            confirmButton = {
    ...
    Scaffold(
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = { showHelp = true }) {
                        Icon(Icons.Default.Info, contentDescription = "說明")
                    }
                },
            )
        },
        ...
        Column(
            ...
            OutlinedButton(
                onClick = onNavigateToCorrectionMemory,
                modifier = Modifier.fillMaxWidth(),
            ) {
                Text("🧠 個人學習記憶：管理錯詞修正、信心度、備份與還原")
            }
            ...
            Text(
                text = "⭐ 重要詞：${importantWords.size} 個",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
                modifier = Modifier.padding(top = 4.dp),
            )
            Text(
                text = "重要詞會優先用於語音辨識與 AI 潤稿",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            ...
            if (entries.isEmpty()) {
                ...
                LazyColumn {
                    items(
                        ...
                        val isImportant = entry.word in importantWords
                        Row(
                            modifier =
                                Modifier
                                    .fillMaxWidth()
                            ...
                            IconButton(
                                onClick = { viewModel.toggleImportantWord(entry.word) },
                            ) {
                                Icon(
                                    imageVector =
                                        if (isImportant) {
                                    ...
                                    contentDescription =
                                        if (isImportant) "取消重要詞" else "設為重要詞",
                                    tint =
                                        if (isImportant) {
